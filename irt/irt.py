...
class IRT(object):
    ...
    def iterate(self):
        ...
        if os.path.exists("IRT.txt"):
            os.remove("IRT.txt")
        while current:
            with open('IRT.txt', 'a+') as f:
...
for i in range(len(post)):
    ...
    if post[i].name[1] == 'callout':
        ...
        instructions.append(pseudo_load_store('li', 'v0', '4'))
        instructions.append(pseudo_load_store('la', 'a0', 'string'+str(len(strings))))
...
with open('IRT.txt', 'a+') as f:
    for i in range(len(strings)):
        f.write('string'+str(i)+': '+strings[i])
